...
int main(void)
{
	...
	cudaMalloc(&dC[G2D_B2D], sizeof(float) * MAT_SIZE); cudaMemset(dC[G2D_B2D], 0, sizeof(float) * MAT_SIZE);
	cudaMalloc(&dC[G1D_B1D], sizeof(float) * MAT_SIZE); cudaMemset(dC[G1D_B1D], 0, sizeof(float) * MAT_SIZE);
	cudaMalloc(&dC[G2D_B1D], sizeof(float) * MAT_SIZE); cudaMemset(dC[G2D_B1D], 0, sizeof(float) * MAT_SIZE);
	...
	// *** Kernel call
	timer.onTimer(5);
	MatAddGPU_2D2D(dA, dB, dC[G2D_B2D]); cudaDeviceSynchronize();
	timer.offTimer(5);
	timer.onTimer(3);
	cudaMemcpy(C[G2D_B2D], dC[G2D_B2D], sizeof(float)* MAT_SIZE, cudaMemcpyDeviceToHost);
	timer.offTimer(3);

	timer.onTimer(6);
	MatAddGPU_1D1D(dA, dB, dC[G1D_B1D]); cudaDeviceSynchronize();
	timer.offTimer(6);
	cudaMemcpy(C[G1D_B1D], dC[G1D_B1D], sizeof(float)* MAT_SIZE, cudaMemcpyDeviceToHost);

	timer.onTimer(7);
	MatAddGPU_2D1D(dA, dB, dC[G2D_B1D]); cudaDeviceSynchronize();
	timer.offTimer(7);
	cudaMemcpy(C[G2D_B1D], dC[G2D_B1D], sizeof(float)* MAT_SIZE, cudaMemcpyDeviceToHost);
...
bool MatAddGPU_2D2D(float* _dA, float* _dB, float* _dC)
{
	dim3 blockDim(32, 32); // 2D
	dim3 gridDim(ceil((float)COL_SIZE/blockDim.x), ceil((float)ROW_SIZE/blockDim.y)); // 2D
...
{
	dim3 blockDim(32); // 1D
	dim3 gridDim(ceil((float)COL_SIZE/blockDim.x)); // 1D
...
{
	dim3 blockDim(32); // 1D
	dim3 gridDim(ceil((float)COL_SIZE/blockDim.x), ROW_SIZE); // 2D
