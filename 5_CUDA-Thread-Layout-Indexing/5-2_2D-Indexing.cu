...
int main(void)
{
    ...
    int vecSize = sizeof(float) * matSize;

	float* dA = NULL;
	float* dB = NULL;
	float* dC = NULL;
    
	cudaMalloc(&dA, vecSize); cudaMemset(dA, 0, vecSize);
	cudaMalloc(&dB, vecSize); cudaMemset(dB, 0, vecSize);
	cudaMalloc(&dC, vecSize); cudaMemset(dC, 0, vecSize);
	...
	cudaMemcpy(dA, A, vecSize, cudaMemcpyHostToDevice);
	cudaMemcpy(dB, B, vecSize, cudaMemcpyHostToDevice);
	...
	cudaMemcpy(hC, dC, vecSize, cudaMemcpyDeviceToHost);
